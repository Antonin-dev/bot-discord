      ...
      if (message.author.id !== this.client.user.id) {
        if (message.mentions.has(this.client.user.id)) {
          ...
          message.channel.send(
            `${this.config.sentenceOfTheDay} ${
              this.randomSentence[
                Math.floor(Math.random() * (this.randomSentence.length - 0))
              ]
            }`
          );
          ...
          message.channel.send(
            this.formatRandomNumber(Number(min), Number(max))
          );
          ...
          if (firstname === 'Antonin') {
            ...
            message.channel.send(
              `${firstname} a copié sur Antonin il/elle mérite un 0/20`
            );
          }
        }
      }
